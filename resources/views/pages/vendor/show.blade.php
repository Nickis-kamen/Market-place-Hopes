<x-layout title="Détails du vendeur">
            ...
            <div class="flex flex-col md:flex-row items-center md:items-start md:space-x-8">
                <div class="mb-4 md:mb-0">
                    <img src="/storage/{{ $vendor->image }}"
                         alt="Photo de {{ $vendor->name }}"
                         class="w-32 h-32 rounded-full object-cover border-4 border-blue-100 shadow" />
                </div>

                <div class="text-center md:text-left">
                    <h2 class="text-3xl font-extrabold text-gray-800">{{ $vendor->name }}</h2>
                    <h3 class="text-xl text-gray-600 font-semibold">{{ $vendor->first_name }}</h3>
                    <p class="text-gray-500 mt-1 flex gap-1"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        ...
                        {{ $vendor->email }}</p>
                    <p class="text-sm text-gray-400 mt-1">Rôle : <span class="uppercase font-semibold text-indigo-600">{{ $vendor->role =='customer'? 'Client':'Vendeur'  }}</span></p>
                    <p class="text-sm text-gray-400">Inscrit le : {{ $vendor->created_at->translatedFormat('d F Y') }}</p>
                </div>
            </div>
            ...
            <div class="mt-8 border-t pt-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-600">
                    <div>
                        ...
                        <p>{{ $vendor->address ?? 'Non renseignée' }}</p>
                    </div>
                    <div>
                        <span class="font-medium">Téléphone :</span>
                        <p>{{ $vendor->phone ?? 'Non renseigné' }}</p>
                    </div>
                    <div>
                        <span class="font-medium">Date de naissance :</span>
                        <p>{{ $vendor->date_naissance->IsoFormat('DD MMMM YYYY') }}</p>
                    </div>
